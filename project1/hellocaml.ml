...
let rev_t (l: 'a list) : 'a list =
  let rec help remain result = 
    begin match remain with
      | [] -> []
      | hd::tl -> help tl result@[hd]
    end 
    in help l []
...
let rec interpret (c:ctxt) (e:exp) : int32 =
  begin match e with
    | Const (x) -> x
    | Var (x) -> lookup x c
    | Add (Const x, Const y) -> Int32.add x y
    | Add (Const x, y) -> Int32.add x (interpret c y)
    | Add (x, y) -> Int32.add (interpret c x) (interpret c y)
    | Mult (Const x, Const y) -> Int32.mul x y
    | Mult (Const x, y) -> Int32.mul x (interpret c y)
    | Mult (x, y) -> Int32.mul (interpret c x) (interpret c y)
    | Neg (Const x) -> Int32.neg x
...
let rec optimize (e:exp) : exp =
  begin match e with 
    ...
    | Add (Const x,y) -> optimize (Add (Const x, optimize y))
    | Add (x,Const y) -> optimize (Add (optimize x,Const y))
    | Mult (Const 0l, y) -> Const (0l)
    | Mult (x,Const 0l) -> Const (0l)
    | Mult (Const 1l, y) -> optimize y
    | Mult (x,Const 1l) -> optimize x
    | Mult (Const x,y) -> optimize (Mult (Const x, optimize y))
    | Mult (x, Const y) ->optimize (Mult (optimize x, Const y))
    | any -> any
  end
